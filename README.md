...
![带证据的 RAG 回答、证据表与运行详情1](docs/assets/rag_answer_evidence_debug.png)
...
这组两张截图放在一起展示用户上传新论文 PDF 的完整链路：先在侧边栏选择 PDF，再点击“处理并更新知识库”完成会话级 upload collection 索引，最后基于上传文献进行增量问答。上传文献只进入会话上传索引，不污染基础文献库。

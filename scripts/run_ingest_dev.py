...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the dev PDF ingest pipeline through chunking.")
    parser.add_argument("--input", type=Path, default=Path("data/dev_papers"))
...
def main() -> int:
    args = parse_args()
    logger, log_path = create_logger("run_ingest_dev")
    try:
        input_dir = resolve_project_path(args.input)
        require_full_run_confirmation(input_dir, args.confirm_full_run)
        summary = run_ingest_pipeline(input_dir, dev_paths(), args.max_files, logger)
